...
const EMPLOYEES_OVERRIDE_KEY = 'ds_employees_override';

export const isDemoMode = (): boolean => {
  const flag = localStorage.getItem(DEMO_MODE_KEY);
  if (flag === 'false') return false;
  if (flag === 'true') return true;
  // Default to false to use local JSON data paths by default
  return false;
};
...
export const resetDemoData = (): void => {
  ...
  localStorage.removeItem(EMPLOYEES_OVERRIDE_KEY);
...
export const loadDemoOverrides = () => {
  ...
  const employees = localStorage.getItem(EMPLOYEES_OVERRIDE_KEY);
  return {
...
export const persistDemoOverrides = (data: {
  ...
  if (data.employees) localStorage.setItem(EMPLOYEES_OVERRIDE_KEY, JSON.stringify(data.employees));
};
