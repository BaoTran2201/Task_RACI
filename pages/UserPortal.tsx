...
import { CheckSquare, Clock, Plus, Edit2, User, Briefcase, Filter, ChevronRight } from 'lucide-react';
import { EditableMyTasksTable } from './EditableMyTasksTable';
import { raciApi } from '../src/services/api';
...
interface UserPortalProps {
  ...
  view: 'dashboard' | 'my_tasks' | 'my_raci' | 'profile';
      ...
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={`whitespace-nowrap px-4 py-2 text-sm font-semibold rounded-t-lg border-b-2 transition-colors ${
              isActive
                        ...
                        <div key={role} className="space-y-2">
                          ...
                          <div className="flex flex-wrap gap-2">
                                ...
                                >
                                  ...
                                  <button
                                    ...
                                    className={`font-bold text-sm leading-none ${
                                      isTaskRoleLoading(task.id, `remove-${position.id}`) || disabledReason
                            ...
                            <button
                              ...
                              className={`inline-flex items-center justify-center w-7 h-7 rounded-full border-2 border-dashed transition-colors font-bold ${
                                isTaskRoleLoading(task.id, `${role}-add`) || !canAddRole(task.id, role)
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <button
                                    ...
                                    disabled={isTaskRoleLoading(task.id, `remove-${position.id}`) || !!disabledReason}
                                    title={disabledReason || 'Xóa'}
                                    className={`ml-1 font-bold text-sm leading-none ${
                                      isTaskRoleLoading(task.id, `remove-${position.id}`) || disabledReason
                                        ? 'text-white/40 cursor-not-allowed'
                                        : 'text-white/80 hover:text-white cursor-pointer'
                                    }`}
...
export const UserPortal: React.FC<UserPortalProps> = (props) => {
  ...
  return (
    <div className="w-full px-4 md:px-6 lg:px-8">
      ...
      {!['dashboard', 'my_tasks', 'my_raci', 'profile'].includes(view) && <div>Page not found</div>}
    </div>
  );
};
